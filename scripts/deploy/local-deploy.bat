@echo off
title Attendance System - Local One Click Deployment
color 0B
echo.
echo ========================================
echo   Attendance System Local One-Click Deploy
echo ========================================
echo.
echo This script will deploy the application from the current directory
echo without downloading from GitHub.
echo.
...
if /i not "%CURRENT_DIR%"=="%INSTALL_DIR%" (
    echo [INFO] Copying files from %CURRENT_DIR% to %INSTALL_DIR%...
    xcopy "%CURRENT_DIR%\*" "%INSTALL_DIR%\" /E /H /C /I /Y
...
netsh advfirewall firewall add rule name="Attendance System" dir=in action=allow protocol=TCP localport=3000 >nul 2>&1
if %errorLevel% equ 0 (
...
set IP_ADDR=%IP_ADDR: =%
echo [INFO] Your IP address: %IP_ADDR%
...
echo.
echo IMPORTANT:
echo 1. Please edit %INSTALL_DIR%\.env.production
echo    and update your Supabase credentials!
